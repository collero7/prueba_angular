...
app.factory('Funciones', ['$http', '$q', function ($http, $q) {
			...
			var self = {
						...
						peliculasIndex: function peliculasIndex(pos) {
						...
						validarNombreApellidos: function validarNombreApellidos(valorIntroducido) {
						...
						validarCorreo: function validarCorreo(correo) {
									...
									if (!expr.test(correo)) {
												//Si el correo introducido no es correcto
												resultado.valido = false;
						...
						validarContrasena: function validarContrasena(contrasena) {
						...
						gestionFavorito: function gestionFavorito(usuarioLogin, peliculaTitulo, pos) {
									...
									for (var i = 0; i < localStorage.length; i++) {
												//Bucle que recorre todas los favoritos
												var favObtenido = localStorage.getItem(usuarioLogin.concat(i + 1));
												if (favObtenido === peliculaTitulo) {
															//Si la pelicula ya se encuentra añadida para ese usuario
									...
									if (datos.ok === true) {
												//Si todo es correcto, creamos el favorito
						...
						crearFavorito: function crearFavorito(usuario, peliculaTitulo) {
						...
						numeroFavoritos: function numeroFavoritos(usuarioUrl) {

									var usuario;
									var numero = 0;

									for (var i = 0; i < localStorage.length; i++) {
												//Recorremos todos los favoritos
												usuario = localStorage.key(i); //Obtenemos todos los favoritos
												if (usuario.substr(0, 4) === usuarioUrl.substr(0, 4)) {
															//Comprobamos todos los favoritos según el usuario
						...
						obtenerFavoritos: function obtenerFavoritos(usuarioUrl) {

									var usuario;
									var contenido = [];

									for (i = 0; i < localStorage.length; i++) {
												//Recorremmos todos los favoritos
						...
						mostrarFavoritos: function mostrarFavoritos(numeroFavoritos, obtenerFavoritos) {

									var contenidoPeliculas = [];

									for (var i = 0; i < numeroFavoritos; i++) {
												//Listamos las películas definidas en la funcion peliculasIndex()

												$http.get(self.URL + 't=' + encodeURI(obtenerFavoritos[i]) + self.KEY).success(function (datos) {
						...
						eliminarFavorito: function eliminarFavorito(usuarioUrl, nombrePelicula) {
						...
						verificarBusquedaPelicula: function verificarBusquedaPelicula(usuarioUrl, peliculaBuscada) {

									$http.get(self.URL + 't=' + encodeURI(peliculaBuscada) + self.KEY).success(function (datos) {
						...
						mostrarBusqueda: function mostrarBusqueda(pagMostrar, peliculaBuscada) {
									...
									$http.get(self.URL + 's=' + encodeURI(peliculaBuscada) + '&page=' + pagMostrar + self.KEY).success(function (datos) {
